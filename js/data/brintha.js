...
randomIndex = getRandomNumber(inputData.length),
grand_parent= inputData[randomIndex]['grand_parent'],
sub=inputData[randomIndex]['sub'],
child=inputData[randomIndex]['child'],
year=inputData[randomIndex]['year'],
pa=inputData[randomIndex]['pa'],
son=inputData[randomIndex]['son'],
family,

question = {};
grand_parent=parseInt(grand_parent);
sub=parseInt(sub);
child=parseInt(child);
year=parseInt(year);
pa=parseInt(pa);
son=parseInt(son);
family=((year*grand_parent)+(pa*sub)+(son*child)/6);

question={
	'type':'single',
	'id':'024',
	'question':'A family consists of '+ grand_parent +'  grand_parent, '+  sub +'  parents and '+ child+' three children the average age of the grand parents is  '+year+' years that ofthe parent is '+pa +' and that of the grandchildren is '+ son+' what is the average age of the family' ,
	'options':{
		'A':family,
...
function problem025(){
    ...
    randomIndex = getRandomNumber(inputData.length),
    person= inputData[randomIndex]['person'],
    increased= inputData[randomIndex]['increased'],
    weight=inputData[randomIndex]['weight'],
    new,
    question = {};
	
	person=parseInt(person);
	increased=parseFloat(increased);
	weight=parseInt(weight);
	new=(person*increased)+weight;
	
	question={
		'type':'single',
		'id':'025',
		'question':' the average weight of the '+ person+'  increased by '+ increased+' kg when a new person comes in place of one of them weighting '+ weight+' of the new person,'
		'option':
		{
			'A':new - parseInt(Math.random()*10),
			'B':new + parseInt(Math.random()*10),
			'C':new,			
			'D':new - parseInt(Math.random()*10),
		},
		'answer':'C',
		'author-id':'025'
		}
...
// program 12
		  ...
		  function problem212(){
		   ...
		   randomIndex = getRandomNumber(inputData.length),
		   pipe-A=inputData[randomIndex]['pipe-A'],
		   pipe-B=inputData[randomIndex]['pipe-B'],
		   fill_tank,
		   question={};
		   
		   pipe-A=parseInt(1/pipe-A);
		   pipe-B=parseInt(1/pipe-B);
		   fill_tank=(pipe-A + pipe-B);
		   question ={
			   'type':'single',
			   'id':'212'
			   'question':'Two pipes A and B can fill a tank in '+ pipe-A + ' hours and '+ pipe-B +' hours respectively.If both the pipes are opened simultaneously, how much time will be taken to fill the tank',
			   'options':
			   'A':fill_tank,
...
function problem219(){
   ...
   randomIndex = getRandomNumber(inputData.length),
	...
	inversment=inputData[randomIndex]['inversement'],
	total,
	question={};
	income=parseInt(income);
	stock=parseInt(stock);
	inversement=parseInt(inversement);
	total=(inversement/stock)*income;
	
	question={
		'type':'single',
		'id':'219',
		'question':'In order to obtain an income of '+ income +' from '+ stock +' at '+inversement +' one ost an inversement of',
		'option':
		'A':total - parseInt(Math.random()*10), 
